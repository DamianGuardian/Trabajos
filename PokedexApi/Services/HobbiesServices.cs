...
public class HobbiesService : IHobbiesService
{
    ...
    public async Task<IHobbiesService?> GetHobbieById(int id, CancellationToken cancellationToken)
    {
        return await _hobbieRepository.GetHobbyBYIdAsync(id, cancellationToken);
    }

    public async Task<List<IHobbiesService>> GetHobbiesByName(string name, CancellationToken cancellationToken)
    {
