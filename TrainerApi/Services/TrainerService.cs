...
public class TrainerService : TrainerApi.TrainerService.TrainerServiceBase
{
    ...
    public override async Task GetTrainersByName(GetTrainersByNameRequest request,
    IServerStreamWriter<TrainerResponse> responseStream, ServerCallContext context)
    {
        if (request.Name.Length <= 1)
        {
            throw new RpcException(
                new Status(StatusCode.InvalidArgument, "Name field is required"));
        }
